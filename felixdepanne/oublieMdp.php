...
$req = $req->fetch();
	...
	if($req['nom'] == $NomModif){
		if($req['prenom'] == $PrenomModif){
			if($req['email'] == $MailModif){
				$reqId = $DB->query("SELECT id from user where nom = '$NomModif' and prenom = '$PrenomModif' and email = '$MailModif'");  
				$reqId = $reqId->fetch();
				$valid = true;
			}
		}
...
//Remplace le mdp utilisateur par le mdp aleatoire
$changepwd=("UPDATE user SET password = :newpassword where id = :id");
$DB->insert($changepwd, array('id' => $reqId['id'], 'newpassword' => (crypt($NewPassword, '$2a$10$1qAz2wSx3eDc4rFv5tGb5t'))));
...
//Envoi mail à l'utilisateur avec le mdp aleatoire
        ...
        $objet = 'Reinitialisation de mot de passe'; // Objet du message
        ...
        {
          $_SESSION['flash']['success'] = "Un mot de passe provisoire vous a été envoyé par email.";
        }
        else // Non envoyé
        {
